...
def _demons_3d(data_in,data_target,m,parms, direction='inverse'):
    ...
    for i in range(0,num_gates):
        ...
        fixed_points = get_all_points(fixed)
        moving_points = get_all_points(moving)

        initial_transform = sitk.Euler3DTransform()
        initial_errors_mean, initial_errors_std, initial_errors, initial_errors_max = registration_errors(
            initial_transform, fixed_points, moving_points
        )
        final_errors_mean, final_errors_std, final_errors, final_errors_max = registration_errors(
            tx, fixed_points, moving_points
        )

        # Create a new figure for each gate
        plt.figure()
        ...
        plt.legend()
        plt.title(f'TRE Histogram for Gate {i}')
        plt.xlabel('Error (mm)')
        plt.ylabel('Frequency')

        text_str = (f"Initial alignment errors in millimeters, mean(std): {initial_errors_mean:.2f}({initial_errors_std:.2f}), max: {initial_errors_max:.2f}\n",
                    f"Final alignment errors in millimeters, mean(std): {final_errors_mean:.2f}({final_errors_std:.2f}), max: {final_errors_max:.2f}")

        plt.figtext(0.5, -0.1, text_str, wrap=True, horizontalalignment='center', fontsize=12, bbox={"facecolor": "white", "alpha": 0.5, "pad": 5})
        plt.tight_layout()
        plt.savefig(f"gate_{i}_tre_hist.png")
        plt.close()  
...
############## ADDED CODE: Get all points to compute registration error ##############################
def get_all_points(image: sitk.Image) -> np.ndarray:
    """
    Extract all voxel coordinates from a SimpleITK image.
    """
    size = image.GetSize()
    spacing = image.GetSpacing()
    origin = image.GetOrigin()

    x, y, z = np.meshgrid(np.arange(size[0]), np.arange(size[1]), np.arange(size[2]), indexing='ij')
    points = np.vstack([x.ravel(), y.ravel(), z.ravel()]).T
    points = points * spacing + origin
    return points


def registration_errors(transform, fixed_points, moving_points):
    """
    Compute registration errors between fixed and moving points.
    """
    # Transform each point individually
    transformed_points = np.array([transform.TransformPoint(pt) for pt in fixed_points])

    # Compute the errors
    errors = np.linalg.norm(transformed_points - moving_points, axis=1)

    mean_error = np.mean(errors)
    std_error = np.std(errors)
    max_error = np.max(errors)

    return mean_error, std_error, errors, max_error
#######################################################################################################
