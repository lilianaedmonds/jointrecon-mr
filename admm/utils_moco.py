...
def phase_based_gating(signal, num_gates):
    # phase-based gating:
    # distribute data between adjacent maxima and minima evenly to the gates
    from scipy.signal import argrelextrema

    maxima = np.asarray(argrelextrema(signal,np.greater,order=15)).squeeze()
    minima = np.asarray(argrelextrema(signal,np.less,order=15)).squeeze()
